...
import { GitHubWidget } from '../../base-items/widgets/GitHubWidget';
...
export const SortableGitHub: React.FC<Props> = ({ id, editMode, isOverlay = false, config, onDelete, onEdit, onDuplicate }) => {
    ...
    return (
        ...
        >
            <WidgetContainer editMode={editMode} id={id} onDelete={onDelete} onEdit={onEdit} onDuplicate={onDuplicate}>
                <GitHubWidget config={config} editMode={editMode} id={id} />
            </WidgetContainer>
        </Box>
    );
};
