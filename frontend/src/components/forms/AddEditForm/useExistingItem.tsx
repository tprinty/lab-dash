...
export const useExistingItem = ({ existingItem, formContext, setCustomIconFile }: UseExistingItemProps) => {
    useEffect(() => {
        ...
        formContext.reset({
            ...
            proxmoxTokenId: existingItem?.type === ITEM_TYPE.PROXMOX_WIDGET ? (existingItem?.config?.proxmoxTokenId || '') : '',
            proxmoxTokenSecret: existingItem?.type === ITEM_TYPE.PROXMOX_WIDGET ? (existingItem?.config?.proxmoxTokenSecret || '') : '',
